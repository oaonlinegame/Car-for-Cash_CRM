(function (global) {
  ...
  // --------------------------------------------------------
  // Component สำหรับจัดการข้อมูลพื้นฐานลูกค้า (ชื่อ, ที่อยู่, อาชีพ)
  // --------------------------------------------------------

  const template = `
    <v-card id="section-lead-main" variant="outlined" rounded="xl" class="section-card section-card--lead-main">
        ...
        <v-card-text class="pt-4 pb-3">
            ...
            <v-col cols="12" md="6" class="d-flex align-center">
                <v-switch color="primary" inset label="ลูกค้าใหม่"
                    v-model="lead.isProspect" hide-details></v-switch>
            </v-col>
        </v-card-text>
    </v-card>
    `;
  ...
  global.registerLeadMainInfo = function (app) {
    app.component("lead-main-info", {
      template: template,
      props: ["lead"], // รับค่า Object ลูกค้าเข้ามา
      setup() {
        // ส่งคืน Store และ LeadApp เพื่อให้ Template เรียกใช้ได้
        return {
          Store,
          LeadApp,
        };
